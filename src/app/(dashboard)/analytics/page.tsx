...
import {
  ...
  BarChart2, Zap, ChevronRight, ExternalLink,
} from "lucide-react";
import {
...
function HourTooltip({ active, payload, label }: TooltipProps) {
  if (!active || !payload?.length) return null;
  return (
    <div className="rounded-xl border border-[rgba(255,255,255,0.1)] bg-[#111420] shadow-xl px-3 py-2 text-xs">
      <p className="text-[#f1f3f9] font-medium">{label}</p>
      <p className="text-[#8892aa]">Engagement score: <span className="text-[#818cf8] font-bold">{payload[0].value}</span></p>
    </div>
  );
}
  ...
  return (
    <div className="card-base p-5">
      <div className="flex items-start justify-between mb-4">
        ...
        <div className={cn("flex items-center gap-0.5 rounded-full px-2 py-0.5 text-2xs font-medium",
          up ? "bg-[rgba(52,211,153,0.12)] text-[#34d399]" : "bg-[rgba(248,113,113,0.12)] text-[#f87171]")}>
          {up ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
          {Math.abs(delta)}%
        </div>
      </div>
      <p className="text-2xl font-bold text-[#f1f3f9] tracking-tight">{value}</p>
      <p className="text-xs text-[#4d5675] mt-0.5">{label}</p>
      {sub && <p className="text-2xs text-[#4d5675] mt-1">{sub}</p>}
    </div>
  );
}
  ...
  return (
    <div className="space-y-5 max-w-[1400px]">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-[#f1f3f9]">Analytics</h2>
          <p className="text-xs text-[#4d5675]">Performance across all connected platforms</p>
        </div>
          ...
          {RANGES.map((r) => (
            <button key={r} onClick={() => setRange(r)}
              className={cn("rounded-lg px-3 py-1.5 text-xs font-medium transition-all",
                range === r ? "bg-[rgba(255,255,255,0.08)] text-[#f1f3f9]" : "text-[#4d5675] hover:text-[#8892aa]"
              )}>
              {r}
            </button>
          ))}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0 }}>
          <StatCard label="Total reach" value={formatNumber(totalReach)} delta={deltas.reach}
            icon={Eye} iconColor="#818cf8" iconBg="rgba(97,114,243,0.12)" sub="Unique accounts reached" />
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          <StatCard label="Engagements" value={formatNumber(totalEng)} delta={deltas.engagements}
            icon={Heart} iconColor="#f9a8d4" iconBg="rgba(249,168,212,0.12)" sub="Likes, comments, shares" />
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <StatCard label="Followers gained" value={`+${formatNumber(totalFol)}`} delta={deltas.followers}
            icon={Users} iconColor="#60a5fa" iconBg="rgba(96,165,250,0.12)" sub="New followers this period" />
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
          <StatCard label="Engagement rate" value={`${engRate}%`} delta={deltas.posts}
            icon={TrendingUp} iconColor="#34d399" iconBg="rgba(52,211,153,0.12)" sub="Avg across all posts" />
        </motion.div>
      </div>
      ...
      <div className="card-base p-5">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h3 className="text-sm font-semibold text-[#f1f3f9]">Performance over time</h3>
            <p className="text-xs text-[#4d5675]">All platforms combined</p>
          </div>
          <div className="flex gap-1">
            {(["reach", "engagements", "followers"] as const).map((m) => (
              <button key={m} onClick={() => setMetric(m)}
                className={cn("rounded-lg px-2.5 py-1 text-xs font-medium capitalize transition-all",
                  metric === m
        ...
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={data} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
            ...
            <XAxis dataKey="label" tick={{ fill: "#4d5675", fontSize: 11 }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fill: "#4d5675", fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={formatNumber} />
            <Tooltip content={<ChartTooltip />} />
            <Area type="monotone" dataKey={metric} stroke="#6172f3" strokeWidth={2}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        {/* Platform breakdown */}
        <div className="card-base p-5">
          <h3 className="text-sm font-semibold text-[#f1f3f9] mb-4">Platform breakdown</h3>
                ...
                <div key={p.name}>
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-medium text-[#f1f3f9]">{p.name}</span>
                    </div>
                    <span className="text-xs text-[#4d5675]">{formatNumber(p.reach)} reach</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-1.5">
                    <span className="text-2xs text-[#4d5675]">{p.posts} posts · {formatNumber(p.followers)} followers</span>
          ...
          <div className="mt-5 pt-4 border-t border-[rgba(255,255,255,0.05)]">
            <p className="text-2xs font-medium text-[#4d5675] mb-3">Best performing platform</p>
            <div className="flex items-center gap-2.5 rounded-xl bg-[rgba(225,48,108,0.08)] border border-[rgba(225,48,108,0.15)] p-3">
              <PlatformIcon platform="INSTAGRAM" size={14} showBg />
              <div>
                <p className="text-xs font-semibold text-[#f1f3f9]">Instagram</p>
                <p className="text-2xs text-[#4d5675]">6.8% engagement rate · highest of all platforms</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 card-base p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-[#f1f3f9]">Top performing posts</h3>
            <span className="text-xs text-[#4d5675]">Last {range}</span>
          </div>
          <div className="space-y-2.5">
            {TOP_POSTS.map((post, i) => (
              <motion.div key={post.id}
                ...
                className="group flex items-start gap-3 rounded-xl border border-[rgba(255,255,255,0.06)] bg-[rgba(255,255,255,0.02)] p-3.5 hover:border-[rgba(255,255,255,0.12)] hover:bg-[rgba(255,255,255,0.03)] transition-all cursor-pointer">
                <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-[rgba(97,114,243,0.12)] text-xs font-bold text-[#818cf8]">
                  {i + 1}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-[#c4cbdc] leading-relaxed line-clamp-2">{post.content}</p>
                  <div className="flex items-center gap-3 mt-2">
                    <div className="flex items-center gap-1">
                      <PlatformIcon platform={post.platform} size={11} />
                    </div>
                    <span className="text-2xs text-[#4d5675]">{post.date}</span>
                    <div className="flex items-center gap-1 text-2xs text-[#4d5675]">
                ...
                <div className="shrink-0 text-right">
                  <div className="text-sm font-bold text-[#10b981]">{post.engRate}%</div>
                  <div className="text-2xs text-[#4d5675]">eng. rate</div>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 card-base p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h3 className="text-sm font-semibold text-[#f1f3f9]">Best times to post</h3>
              <p className="text-xs text-[#4d5675]">Based on your audience engagement patterns</p>
            </div>
            <div className="flex items-center gap-1.5 rounded-full bg-[rgba(97,114,243,0.1)] border border-[rgba(97,114,243,0.2)] px-2.5 py-1">
              <Zap className="h-3 w-3 text-[#818cf8]" />
              <span className="text-2xs font-medium text-[#818cf8]">AI optimized</span>
            </div>
          </div>
          <ResponsiveContainer width="100%" height={140}>
            <BarChart data={BEST_HOURS} margin={{ top: 0, right: 0, left: -28, bottom: 0 }}>
              <XAxis dataKey="hour" tick={{ fill: "#4d5675", fontSize: 10 }} axisLine={false} tickLine={false} />
              <YAxis hide />
          ...
          <div className="mt-3 flex items-center gap-4">
            {[
              ...
              <div key={t.time} className="flex items-center gap-2 rounded-xl bg-[rgba(97,114,243,0.08)] border border-[rgba(97,114,243,0.15)] px-3 py-2">
                <div className="h-2 w-2 rounded-full bg-[#6172f3]" />
                <span className="text-xs font-semibold text-[#f1f3f9]">{t.time}</span>
                <span className="text-2xs text-[#4d5675]">{t.label} · score {t.score}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Quick insights */}
        <div className="card-base p-5">
          <h3 className="text-sm font-semibold text-[#f1f3f9] mb-4">Quick insights</h3>
          <div className="space-y-3">
            {[
              { icon: TrendingUp, color: "text-[#34d399]", bg: "rgba(52,211,153,0.1)", title: "Reach up 24%", desc: "Instagram driving the growth this week" },
              { icon: Heart,      color: "text-[#f9a8d4]", bg: "rgba(249,168,212,0.1)", title: "Best engagement day: Saturday", desc: "Posts at 6 PM get 3× more likes" },
              { icon: BarChart2,  color: "text-[#818cf8]", bg: "rgba(97,114,243,0.1)",  title: "51 posts published", desc: "Up 8% from last {range}" },
              { icon: Users,      color: "text-[#60a5fa]", bg: "rgba(96,165,250,0.1)",  title: "+250 new followers", desc: "Twitter growing fastest at 18%/wk" },
            ].map(({ icon: Icon, color, bg, title, desc }) => (
              <div key={title} className="flex items-start gap-3 rounded-xl p-3 hover:bg-[rgba(255,255,255,0.02)] transition-colors">
                <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg" style={{ backgroundColor: bg }}>
                  <Icon className={cn("h-3.5 w-3.5", color)} />
                </div>
                <div>
                  <p className="text-xs font-medium text-[#f1f3f9]">{title}</p>
                  <p className="text-2xs text-[#4d5675] mt-0.5">{desc}</p>
                </div>
              </div>
            ))}
          </div>
          <button className="mt-4 flex w-full items-center justify-center gap-1.5 rounded-xl border border-[rgba(255,255,255,0.08)] py-2.5 text-xs text-[#4d5675] hover:text-[#8892aa] hover:border-[rgba(255,255,255,0.14)] transition-all">
